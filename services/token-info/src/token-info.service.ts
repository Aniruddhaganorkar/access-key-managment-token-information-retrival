...
import { TokenInfo } from './schema/tis.schema';
import { TokenInfoRepository } from './token-info.repository';
import { CreateTisDto, GetTokenDto, UpdateTisFromKmsDto, UpdateTisForTokenDto } from "@app/common/dtos/index";


@Injectable()
export class TokenInfoService {
  constructor(private readonly tokenInfoRepository: TokenInfoRepository) {}
  ...
  async getTisByIdName(username: string, accessKey: string): Promise<TokenInfo> {
    return await this.tokenInfoRepository.findOne({ username, accessKey })
  }

  async getAllTis(): Promise<TokenInfo[]> {
      return this.tokenInfoRepository.find({});
  }

  async createTis(newTis: CreateTisDto): Promise<TokenInfo> {
  ...
  async updateTis(userId: string, tisUpdates: UpdateTisFromKmsDto): Promise<TokenInfo> {
  ...
  async updateTisForToken(tisUpdates: UpdateTisForTokenDto): Promise<TokenInfo> {
    return this.tokenInfoRepository.findOneAndUpdate({ username: tisUpdates.username, accessKey: tisUpdates.accessKey }, tisUpdates);
  }

  async deleteTis(userId: string): Promise<TokenInfo> {
